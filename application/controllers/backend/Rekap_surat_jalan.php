...
*/	
	...
	public function get_ajax_list()
	{
		...
		foreach ($list as $d) {
			...
			$row[] = number_format($d->jumlah_surat_jalan, 0, "", ",");
	...
	public function nota_pdf() {
	    ...
	    $total_jumlah_surat_jalan = number_format($jumlah_surat_jalan, 0, "", ",");
        ...
        foreach ($foreach as $row){
            ...
            $pdf->Cell(70,6,number_format($row->jumlah_ls_surat_jalan, 0, "", ",").' '.$row->nama_satuan,1,1);
        }
        $pdf->SetFont('times','B',12);
        $pdf->Cell(10,6,' ',1,0);
        $pdf->Cell(110,6,' ',1,0);
        $pdf->Cell(70,6,'Total Jumlah :'.$total_jumlah_surat_jalan.' '.$nama_satuan,1,0);
        $pdf->Cell(10,7,'',0,1);
        $pdf->Cell(10,7,'',0,1);
        if ($bretur < 1) {
	...
	public function get_ajax_list_detail()
	{
		...
		foreach ($list as $d) {
			...
			$row[] = $no;
			$row[] = $d->nama_stock;
			$row[] = number_format($d->jumlah_ls_surat_jalan, 0, "", ",");
